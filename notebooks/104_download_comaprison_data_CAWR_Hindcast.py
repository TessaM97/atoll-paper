...
locations = [
    ...
    {"name": "Nukulaelae, Tuvalu", "lat": -9.3872, "lon": 179.8424 , "max": "2015-03-13" },
...
def get_max_wave_event(ds, locations, start_date=None, end_date=None):
    ...
    for loc in locations:
        ...
        hs_sel = ds['hs'].sel(latitude=loc['lat'], longitude=loc['lon'], method="nearest")
        lm_sel = ds['lm'].sel(latitude=loc['lat'], longitude=loc['lon'], method="nearest")
    ...
    print("\nMaximum Wave Events:")
    for r in results:
        print(f"{r['location']} ({r['lat']:.2f}, {r['lon']:.2f}) | "
              f"Date: {r['time']} | HS: {r['max_HS_m']} m | LM: {r['LM_at_max_HS_m']} m")

    return results
    
summary = get_max_wave_event(ds, locations, start_date="2015-03-06", end_date="2015-03-20")
